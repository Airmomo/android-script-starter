...
from config import yamlConfig, adbConfig, logConfig
...
from view.ui.findHelper_ui import UI
from util import imageUtil, logUtil
...
class FindHelperWindow(QMainWindow, UI):
    ...
    def openAdbImage(self):
        ...
        if not devices:
            ...
            android_device = AndroidDevice(adbConfig.ADB_PATH, devices[0], 720, 1280, logConfig.LOGGER_LEVEL)
        else:
    ...
    def saveCropImage(self):
        ...
        try:
            ...
            logUtil.error("[图色工具]", "->保存裁剪区域[截图保存失败/取消保存]")
